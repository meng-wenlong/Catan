...
和好友一起在浏览器里玩的卡坦岛。无需注册，创建房间把 4 位房间码发给朋友即可开玩。语音请自行使用钉钉会议等工具。
...
```bash
npm test                  # 单元测试（棋盘生成 + 游戏规则）
node test/e2e-smoke.js    # 端到端冒烟测试（需先启动服务器）
```
...
server/
  ...
  longestRoad.js  最长道路算法（DFS，含对手建筑截断）
